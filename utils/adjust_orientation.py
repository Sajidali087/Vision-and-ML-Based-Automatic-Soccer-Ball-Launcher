...
class PIDDetector:
    def __init__(self):
        ...
        try:
            self.pid_controller = PIDController(Kp=0.9, Ki=0.005, Kd=0.1, integral_limit=50)
    ...
    def detect_and_control(self, frame, detected_center_x, image_height):
        try:
            ...
            # Compute PID output
            pid_output = self.pid_controller.compute(self.center_x, detected_center_x, 240)
            ...
            self.person_in_front = (self.center_x - self.offset) <= detected_center_x <= (self.center_x + self.offset)

            # Draw center line area
            for i in range(-self.offset, self.offset + 1):
